...
class Custom_Validator extends CI_Controller {
	public static $regexes = Array('date' => "^[0-9]{4}[-/][0-9]{1,2}[-/][0-9]{1,2}\$", 'amount' => "^[-]?[0-9]+\$", 'number' => "^[-]?[0-9,]+\$", 'alfanum' => "^[0-9a-zA-Z ,.-_\\s\?\!]+\$", 'not_empty' => "[a-z0-9A-Z]+", 'words' => "^[A-Za-z]+[A-Za-z \\s]*\$", 'phone' => "^[0-9]{10,11}\$", 'zipcode' => "^[1-9][0-9]{3}[a-zA-Z]{2}\$", 'plate' => "^([0-9a-zA-Z]{2}[-]){2}[0-9a-zA-Z]{2}\$", 'price' => "^[0-9.,]*(([.,][-])|([.,][0-9]{2}))?\$", '2digitopt' => "^\d+(\,\d{2})?\$", '2digitforce' => "^\d+\,\d\d\$", 'anything' => "^[\d\D]{1,}\$", 'username' => "^[\w]{3,32}\$");

	private $validations, $sanatations, $mandatories, $mandatoryOrSets, $equal, $unique, $errors, $corrects, $fields, $ci;

	public function __construct($validations = array(), $mandatories = array(), $mandatoryOrSets = array(), $sanatations = array(), $unique = array(), $equal = array()) {
		$this -> ci = &get_instance();

		$this -> validations = $validations;
		$this -> sanatations = $sanatations;
		$this -> mandatories = $mandatories;
		$this -> mandatoryOrSets = $mandatoryOrSets;
		$this -> unique = $unique;
		$this -> equal = $equal;
		$this -> errors = array();
		$this -> corrects = array();
	...
	public function validate($items) {
		...
		//Check for mandatoryOrSets
		foreach ($this->mandatoryOrSets as $reqOrSet) {
			$hasReqPresent = false;
			$trackPotentialMissing = array();
			foreach ($reqOrSet as $key => $val) {
				if (array_key_exists($val, $items)) {
					$hasReqPresent = true;
				} else {
					array_push($trackPotentialMissing, $val);
				}
			}
			// if none of required or set was fulfilled, add errors and fail validation
			if (!$hasReqPresent) {
				$havefailures = true;
				foreach ($trackPotentialMissing as $key => $val) {
					$this -> addError($val);
				}
			}
		}
