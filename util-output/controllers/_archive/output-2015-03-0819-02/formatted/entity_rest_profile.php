...
class Entity_rest_profile extends REST_Controller {
    ...
    {
        $modelResult = $this -> entity_rest_profile_model -> get_all();
        if (!$modelResult) {
            $this -> response(array("success" => "false", "errorMsg" => "entity_rest_profile does not exist."), 200);
        } else {
    ...
    function index_put()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("success" => "false", "errorMsg" => "entity_rest_profile put request failure."), 200);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_post()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("success" => "false", "errorMsg" => "entity_rest_profile post request failure."), 200);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_delete()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("success" => "false", "errorMsg" => "entity_rest_profile delete request failure."), 200);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
