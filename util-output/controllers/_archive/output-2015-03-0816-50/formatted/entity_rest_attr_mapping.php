...
class Entity_rest_attr_mapping extends REST_Controller {
    ...
    function index_get()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "Sorry, entity_rest_attr_mapping Get request failure (does not exist)."), 404);
            } else {
    ...
    function index_put()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "entity_rest_attr_mapping put request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_post()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "entity_rest_attr_mapping post request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_delete()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "entity_rest_attr_mapping delete request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
