...
class Api extends REST_Controller {
    ...
    function index_get()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "Sorry, api Get request failure (does not exist)."), 404);
            } else {
    ...
    function index_put()
    {
        ...
        $putArray = array("id" => $this -> put("id"), "name" => $this -> put("name"));
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "api put request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_post()
    {
        $postArray = array("name" => $this -> post("name"));
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "api post request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
    ...
    function index_delete()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this -> response(array("error" => "api delete request failure."), 404);
            } else {
                $this -> response($modelResult, 200);
            } 
        } 
