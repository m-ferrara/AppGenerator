...
$VldtnRules = array("id"=>"number", "table_name"=>"alfanum", "name"=>"alfanum", "required_api_key"=>"number", "rest_complete_status"=>"number");$MandatoryRules = array("id");

// Sanatization Rules 

$SntnRules = array("id"=>"number", "table_name"=>"alfanum", "name"=>"alfanum", "required_api_key"=>"number", "rest_complete_status"=>"number");$UniqueRules = array();$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}function postValidator( $RequestPayload )  {

// Validation Rules 

$VldtnRules = array("table_name"=>"alfanum", "name"=>"alfanum", "required_api_key"=>"number", "rest_complete_status"=>"number");$MandatoryRules = array("table_name", "name");

// Sanatization Rules 

$SntnRules = array("table_name"=>"alfanum", "name"=>"alfanum", "required_api_key"=>"number", "rest_complete_status"=>"number");$UniqueRules = array("table_name"=>"entity", "name"=>"entity");$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}function deleteValidator( $RequestPayload )  {
