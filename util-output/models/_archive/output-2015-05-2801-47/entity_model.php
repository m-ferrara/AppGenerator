...
$reqArray = array();$id = $this->put("id");if(is_string($id) && strlen($id) > 0) {$reqArray["id"] = $id; }$table = $this->put("table");if(is_string($table) && strlen($table) > 0) {$reqArray["table"] = $table; }$name = $this->put("name");if(is_string($name) && strlen($name) > 0) {$reqArray["name"] = $name; }$status = $this->put("status");if(is_string($status) && strlen($status) > 0) {$reqArray["status"] = $status; }return $reqArray;}function postAssembler()  {

// Assign Params if Detected 

$reqArray = array();$table = $this->post("table");if(is_string($table) && strlen($table) > 0) {$reqArray["table"] = $table; }$name = $this->post("name");if(is_string($name) && strlen($name) > 0) {$reqArray["name"] = $name; }$status = $this->post("status");if(is_string($status) && strlen($status) > 0) {$reqArray["status"] = $status; }return $reqArray;}function deleteAssembler( )  {
