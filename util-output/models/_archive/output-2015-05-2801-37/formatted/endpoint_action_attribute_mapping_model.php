<?php class Endpoint_action_attribute_mapping_model extends CI_Model {
    ...
    function putAssembler()
    {
        ...
        $acti > put "action_method");if (is_string($action_method) && strlen($action_method) > 0) {
        $reqArray["action_method"] = $action_method; } 
    $ent_id = $this -> put("ent_id");if (is_string($ent_id) && strlen($ent_id) > 0) {
        $reqArray["ent_id"] = $ent_id; } 
    $attr_id = $this -> put("attr_id");if (is_string($attr_id) && strlen($attr_id) > 0) {
        $reqArray["attr_id"] = $attr_id; } 
    $category = $this -> put("category");if (is_string($category) && strlen($category) > 0) {
...
function postAssembler()
{
    ...
    // Assign Params if Detected
    $reqArray = array();$endpt_id = $this -> post("endpt_id");if (is_string($endpt_id) && strlen($endpt_id) > 0) {
        $reqArray["endpt_id"] = $endpt_id; } 
    $attr_id = $this -> post("attr_id");if (is_string($attr_id) && strlen($attr_id) > 0) {
        $reqArray["attr_id"] = $attr_id; } 
    $category = $this -> post("category");if (is_string($category) && strlen($category) > 0) {
        $reqArray["category"] = $category; } 
    $acti > post("action_method");if (is_string($action_method) && strlen($action_method) > 0) {
        $reqArray["action_method"] = $action_method; } 
    return $reqArray;} 
...
function getValidator($RequestPayload)
{
    ...
    // Sanatization Rules
    $SntnRules = array("endpt_id" => "number", "action_method" => "alfanum");$UniqueRules = array();$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator -> validate($RequestPayload)) {
        $sanatizedPayload = $validator -> sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {
...
function deleteValidator($RequestPayload)
{
    ...
    // Sanatization Rules
    $SntnRules = array("id" => "number");$UniqueRules = array();$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator -> validate($RequestPayload)) {
        $sanatizedPayload = $validator -> sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {
        return array("isValid" => false, "errorMsg" => $validator -> getJSON());} 
} 
} 
