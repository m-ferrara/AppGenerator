<?php class Entity_model extends CI_Model {
    ...
    function putValidator($RequestPayload)
    {
        ...
        // Validation Rules
        $VldtnRules = array("id" => "number", "table" => "undefined", "name" => "alfanum", "status" => "undefined");
        $MandatoryRules = array("id");
        
        // Sanatization Rules
        $SntnRules = array("id" => "number", "table" => "undefined", "name" => "alfanum", "status" => "undefined");
        $UniqueRules = array("table" => "entity", "name" => "entity");
        $validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);
        if ($validator -> validate($RequestPayload)) {
            $sanatizedPayload = $validator -> sanatize($RequestPayload);
            return array("isValid" => true, "payload" => $sanatizedPayload);
    ...
    function postValidator($RequestPayload)
    {
        ...
        // Validation Rules
        $VldtnRules = array("table" => "undefined", "name" => "alfanum", "status" => "undefined");
        $MandatoryRules = array("table", "name");
        
        // Sanatization Rules
        $SntnRules = array("table" => "undefined", "name" => "alfanum", "status" => "undefined");
        $UniqueRules = array("table" => "entity", "name" => "entity");
        $validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);
        if ($validator -> validate($RequestPayload)) {
            $sanatizedPayload = $validator -> sanatize($RequestPayload);
