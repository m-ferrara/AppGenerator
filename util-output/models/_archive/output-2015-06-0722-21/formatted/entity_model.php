<?php class Entity_model extends CI_Model {
    ...
    function getValidator($RequestPayload)
    {
        ...
        $UniqueRules = array("undefined" => "entity");
    ...
    function putValidator($RequestPayload)
    {
        ...
        $UniqueRules = array("AND_CONDITIONAL" => array("TABLE" => "entity", "KEYS" => array("table, "name")));$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}function postValidator( $RequestPayload )  {
// Validation Rules 
$VldtnRules = array("table"=>"alfanum", "name"=>"alfanum", "status"=>"number");$MandatoryRules = array("table", "name");
// Sanatization Rules 
$SntnRules = array("table"=>"alfanum", "name"=>"alfanum", "status"=>"number");$UniqueRules = array("AND_CONDITIONAL"=>array("TABLE"=>"entity","KEYS"=>array("table, "name")));
        $validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);
        if ($validator -> validate($RequestPayload)) {
            $sanatizedPayload = $validator -> sanatize($RequestPayload);
            return array("isValid" => true, "payload" => $sanatizedPayload);
