<?php class Api_attr_mapping_model extends CI_Model {
    ...
    function getValidator($RequestPayload)
    {
        ...
        // Validation Rules
        $VldtnRules = array("api_id" => "number");
        $MandatoryRules = array("api_id");
        
        // Sanatization Rules
        $SntnRules = array("api_id" => "number");
        $UniqueRules = array("undefined" => "api_attr_mapping");
    ...
    function postValidator($RequestPayload)
    {
        ...
        $UniqueRules = array("AND_CONDITIONAL" => array("TABLE" => "api_attr_mapping", "KEYS" => array("api_id, "attr_id")));$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}function deleteValidator( $RequestPayload )  {
// Validation Rules 
$VldtnRules = array("id"=>"number");$MandatoryRules = array("id");
// Sanatization Rules 
$SntnRules = array("id"=>"number");$UniqueRules = array("undefined"=>"api_attr_mapping");$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}}/* end of Api_attr_mapping_model.php */