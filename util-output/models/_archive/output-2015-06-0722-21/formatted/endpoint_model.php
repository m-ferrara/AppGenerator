<?php class Endpoint_model extends CI_Model {
    ...
    function getValidator($RequestPayload)
    {
        ...
        $UniqueRules = array("undefined" => "endpoint");
    ...
    function postValidator($RequestPayload)
    {
        ...
        // Sanatization Rules
        $SntnRules = array("ent_id" => "number", "api_id" => "number");
        $UniqueRules = array("AND_CONDITIONAL" => array("TABLE" => "endpoint", "KEYS" => array("ent_id, "api_id")));$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}function deleteValidator( $RequestPayload )  {
// Validation Rules 
$VldtnRules = array("id"=>"number");$MandatoryRules = array("id");
// Sanatization Rules 
$SntnRules = array("id"=>"number");$UniqueRules = array("undefined"=>"endpoint");$validator = new Custom_Validator($VldtnRules, $MandatoryRules, $SntnRules, $UniqueRules);if ($validator->validate($RequestPayload)) {$sanatizedPayload = $validator->sanatize($RequestPayload);return array("isValid" => true, "payload" => $sanatizedPayload);} else {return array("isValid" => false, "errorMsg" => $validator->getJSON());}}}/* end of Endpoint_model.php */